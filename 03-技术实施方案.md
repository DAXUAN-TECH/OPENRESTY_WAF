...
### 3.2 索引设计
- `access_logs.client_ip`：索引，支持按 IP 查询
- `access_logs.request_time`：索引，支持时间范围查询
- `block_rules.status`：索引，只查询启用规则
- `block_rules.rule_type`：索引，按类型查询
...
详见 `05-nginx.conf` 和 `06-waf.conf`
