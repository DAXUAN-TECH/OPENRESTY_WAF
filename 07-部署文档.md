...
# 下载 GeoLite2-City.mmdb 并放到 lua/geoip/ 目录
...
# 创建数据库
CREATE DATABASE waf_db DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
```bash
# 执行 SQL 脚本
mysql -u waf_user -p waf_db < /path/to/04-数据库设计.sql
```
...
```bash
# 创建 Lua 脚本目录
sudo mkdir -p /usr/local/openresty/nginx/lua/waf
...
# 复制 Lua 配置文件
sudo cp lua/config.lua /usr/local/openresty/nginx/lua/config.lua

# 复制 Lua 模块
sudo cp lua/waf/*.lua /usr/local/openresty/nginx/lua/waf/

# 创建 GeoIP2 数据库目录（如果不存在）
sudo mkdir -p /usr/local/openresty/nginx/lua/geoip
# 注意：需要手动下载 GeoLite2-Country.mmdb 并放到此目录
# 参考 lua/geoip/README.md
```

### 4.3 修改配置文件

编辑 `/usr/local/openresty/nginx/lua/config.lua`，修改数据库连接信息：
...
# 检查 MySQL 连接
mysql -u waf_user -p waf_db -e "SELECT COUNT(*) FROM access_logs;"
```
