...
```sql
-- 封控多个国家
INSERT INTO block_rules (rule_type, rule_value, rule_name, description, status, priority) VALUES
...
```sql
-- 封控北京，仅在特定时间段生效
INSERT INTO block_rules (rule_type, rule_value, rule_name, description, status, priority, start_time, end_time)
...
```sql
-- 启用规则
UPDATE block_rules SET status = 1 WHERE id = 1;

-- 禁用规则
UPDATE block_rules SET status = 0 WHERE id = 1;
```
...
2. 查询规则是否已添加：
```sql
SELECT * FROM block_rules WHERE rule_name = '测试-封控北京';
```

3. 启用规则并测试：
```sql
UPDATE block_rules SET status = 1 WHERE rule_name = '测试-封控北京';
```
...
5. 测试完成后删除规则：
```sql
DELETE FROM block_rules WHERE rule_name = '测试-封控北京';
```
