...
- [ ] **日志采集测试**
  ...
  # 检查数据库是否有记录
  mysql -u waf_user -p waf_db -e "SELECT * FROM access_logs ORDER BY id DESC LIMIT 5;"
  ```
...
- [ ] **单个 IP 封控测试**
  ...
  -- 查询规则
  SELECT * FROM block_rules WHERE rule_name = '测试封控';
  
  -- 启用规则
  UPDATE block_rules SET status = 1 WHERE rule_name = '测试封控';
...
- [ ] **IP 段封控测试**
  ...
  -- 启用并测试
  UPDATE block_rules SET status = 1 WHERE rule_name = '测试IP段封控';
  ```
...
- [ ] **地域封控测试**
  ...
  -- 启用并测试
  UPDATE block_rules SET status = 1 WHERE rule_name = '测试-封控美国';
  ```
...
- [ ] **封控日志记录**
  ```bash
  # 触发封控后，检查封控日志
  mysql -u waf_user -p waf_db -e "SELECT * FROM block_logs ORDER BY id DESC LIMIT 5;"
  ```
...
```bash
# 检查规则状态
mysql -u waf_user -p waf_db -e "SELECT * FROM block_rules WHERE status = 1;"
...
- 业务日志：MySQL `access_logs` 和 `block_logs` 表
