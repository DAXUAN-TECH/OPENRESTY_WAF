...
详细说明请参考：[start_README.md](../start_README.md)
...
├── start_README.md              # 一键安装脚本说明
│
├── init_file/                   # 初始配置文件目录
│   ├── nginx.conf               # Nginx 主配置文件（部署时复制到系统目录）
│   └── 数据库设计.sql           # 数据库表结构和初始数据
│
├── conf.d/                      # 配置文件目录（保持在项目目录）⭐
│   ├── set_conf/                # 参数配置文件
│   │   ├── basic.conf           # 基础配置（sendfile、keepalive等）
│   │   ├── gzip.conf            # Gzip 压缩配置
│   │   ├── brotli.conf          # Brotli 压缩配置（可选）
│   │   ├── http2.conf           # HTTP/2 配置（可选）
...
| `init_file/数据库设计.sql` | 仅用于初始化数据库 | 不复制，直接执行 |
...
# 将 include conf.d/set_conf/*.conf 改为 include $project_root/conf.d/set_conf/*.conf
# 将 include conf.d/vhost_conf/*.conf 改为 include $project_root/conf.d/vhost_conf/*.conf
# 注意：使用 $project_root 变量而不是硬编码路径
```
...
**Shell 脚本关联**：
- `start.sh update-config` - 自动更新 MySQL 和 Redis 配置
...
**位置**：`conf.d/set_conf/performance.conf`
...
**位置**：`conf.d/set_conf/gzip.conf`（Gzip）
**位置**：`conf.d/set_conf/brotli.conf`（Brotli，可选）
...
**位置**：`conf.d/set_conf/http2.conf`（可选）
