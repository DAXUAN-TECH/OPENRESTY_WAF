...
**`openresty -s reload` 不会重新生成配置文件**

- `reload_nginx_internal()` 函数只是执行 `openresty -s reload` 命令
- 它只会重新加载磁盘上已存在的配置文件
- **不会**去数据库读取配置并重新生成配置文件
...
- **`openresty -s reload` 不会重新生成配置文件**，只会重新加载现有的配置文件
