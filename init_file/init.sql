...
CREATE TABLE IF NOT EXISTS waf_proxy_configs (
    ...
    backend_address VARCHAR(255) NOT NULL COMMENT '后端地址（单个后端：IP或域名，多个后端：upstream名称）',
    backend_port INT UNSIGNED DEFAULT NULL COMMENT '后端端口（单个后端时使用，HTTP/TCP/UDP代理都需要）',
