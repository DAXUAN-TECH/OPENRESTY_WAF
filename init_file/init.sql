...
CREATE TABLE IF NOT EXISTS waf_proxy_configs (
    ...
    backend_address VARCHAR(255) DEFAULT NULL COMMENT '后端地址（已废弃：现在使用upstream类型，后端服务器信息存储在waf_proxy_backends表中）',
    backend_port INT UNSIGNED DEFAULT NULL COMMENT '后端端口（已废弃：现在使用upstream类型，后端服务器信息存储在waf_proxy_backends表中）',
    backend_path VARCHAR(255) DEFAULT NULL COMMENT '后端路径（HTTP代理时使用，代理到后端的特定路径，如/aaa，留空则代理到根路径）',
