...
#if [ -f "$WAF_ADMIN_SSL_CONF" ] && grep -q "listen.*443.*ssl" "$WAF_ADMIN_SSL_CONF" 2>/dev/null && ! grep -q "ssl_certificate" "$WAF_ADMIN_SSL_CONF" 2>/dev/null; then
#    echo -e "${YELLOW}  检测到 waf_admin_ssl.conf 包含不完整的 SSL 配置，请通过管理端SSL设置页面重新保存配置修复...${NC}"
#fi
